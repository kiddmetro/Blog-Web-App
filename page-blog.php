...
// Retrieve blog ID and title from URL
$blogId = $_GET['blog_id'];
$userId = $_GET['user_id'];
// $userName = $_GET['username'];
$blogTitle = urldecode($_GET['title']);

//  Fetch additional information from the database
$stmt = $pdo->prepare("SELECT image, category, title, author_name, author_image, formatted_date,read_time,sample_text  FROM blogs WHERE blog_id = :blogId");
...
if (!$result) {
    echo "Blog not found";
    // You might want to redirect the user or handle this situation differently
    exit;
}

// Extract data from the result
$blogImage = $result['image'];
$category = $result['category'];
$blogTitle = $result['title'];
$adminName = $result['author_name'];
$adminImage = $result['author_image'];
$readTime = $result['read_time'];
$blogText = $result['sample_text'];
...
<body style="overflow-x: hidden;">
    ...
    <div class="container-lg">            
        <div class="blog-card">
            ...
            <div class=" blog-single mt-4">
                <p><?php echo $blogText; ?></p>
            </div>
        </div> 

        <div class="container-lg" >
            <div class="comment-section-header mt-5">
                <span class="first-text">Comments</span><span class="second-text">Section</span>
            </div>

            <form class="row g-3 " action="./controller/comment-ctrl.php" method="POST"  >
                <div>
                    <h5 class="mt-5">Blog Comment</h5>
                </div>
                <div class="col-12">
                    <textarea id="comment-area" name="commenttext" class="form-sty" placeholder="Write a comment..."></textarea>
                    <input type="hidden" name="blog_id" value="<?php echo $blogId; ?>">
                    <input type="hidden" name="title" value="<?php echo $blogTitle; ?>">
                    <input type="hidden" name="user_id" value="<?php echo $userId; ?>">
                </div>
                <div class="col-12 justify-content-center align-items-center ms-5">
                    <button type="submit" class="btn btn-sm text-light btn-primary" name="comment-submit" >Comment</button>
                </div>
    ...
    <footer class=" mt-auto py-3 footer-bg ">
        <div class="container-lg">
            <div class="footer-wrapper">
                <div>
                    <a class="footer-brand" href="#"><span class="blog-logo">Blog</span><span class="blog-logo2">Book</span><img src="./image/logo/Group 3381.svg" class="logo-image" alt=""></a>
